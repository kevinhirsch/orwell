...
- **GitHub API quota (overseers/agents):** all sessions + the GitHub MCP share ONE per-user 5k/h REST
  pool — `403 rate limit exceeded` is quota (hourly reset), not auth. Verify via local git (zero quota),
  webhook-wake not poll, batch API bursts at the window reset. Full discipline + escalation paths:
  `SOUL.md` lesson 23.
