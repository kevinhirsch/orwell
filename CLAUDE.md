...
| `npm run test:heavy` | Just the two heavy simulation files (full-game UAT + the juryReach calibration gate); CI's parallel `heavy-sims` lane runs them one-file-per-runner via `test:heavy:uat` / `test:heavy:jury`. |
