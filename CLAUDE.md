...
**armed** (2026-07-09: the first real-model fixture `frontend/tests/golden/golden_path_glm-4.7.jsonl` was
committed and regenerated in #1251; the `golden-path` CI job replays it key-free — twice, comparing
digests — and is **advisory / non-gating** (excluded from `ci-gate.needs`, owner ruling 2026-07-12; it still runs + reports); `frontend/INTEGRATION.md` §golden-path holds the one-command record). The 2026-07-06 closure session shipped
...
- **The golden-path re-record obligation (READ THIS before touching any prompt/tool-schema/casting flow).** `golden-path` **and** `visual-regression` **and** `theme-consistency` all replay the SAME committed fixture `frontend/tests/golden/golden_path_glm-4.7.jsonl`. These three are **advisory / non-gating** — **deliberately excluded from `ci-gate.needs` (owner ruling 2026-07-12**; they replay the heavy fixture end-to-end and fail intermittently on slow gh-runners under load, which was blocking the merge lane — see the comment at `ci-gate.needs` in `ci.yml`). So a stale (or hang-tripping) fixture makes the PR `mergeable_state: **unstable**` with these red, **not** `blocked`; it still merges (as #1712 did). They still RUN + report their own advisory status, and `golden-nightly.yml` keeps a fresh fixture. **The re-record obligation still stands regardless:** keep the committed fixture current so these gates are meaningful and so they green once the intermittent-hang root cause (issue #1713 — the casting-finalize → premiere ~300s engine-timeout hang) is fixed. ANY change that shifts a golden request digest — the narrator prompt (`momentPrompts.BASE_GAME_MASTER_PROMPT`), a tool schema, or the casting-finalize flow (e.g. a forced `tool_choice`) — **stales the fixture** and MUST be re-recorded **in the same PR**. The re-record needs a **live OpenRouter key** (every automated gate stubs the LLM): `cd frontend && OPENROUTER_API_KEY=sk-… ORWELL_GOLDEN_RECORD=1 python3 scripts/golden_path_record.py` (defaults: `z-ai/glm-4.7` narration + `qwen/qwen3.6-flash` utility, seed 108108, one driver run at a time — watch for `RECORD OK` + a clean leak scan; a truncated/interrupted record silently corrupts the fixture), then the key-free gate `python3 scripts/golden_path_replay.py --runs 2` must PASS (I1–I8 + R1 zero misses + R2 no provider reach). **Record against the branch merged with current `main`** — CI replays the merge-ref, so a prompt PR merging *after* you record re-stales it (the "fixture treadmill"); land the re-record fast, and if it goes red on arrival, another prompt PR merged and you re-record once more. `frontend/INTEGRATION.md` §golden-path is the one-command record; `golden-nightly.yml` uploads a pre-baked refreshed fixture nightly.

- **`live-harness-nightly.yml`** (2026-07-11, sibling of `golden-nightly.yml`) runs the LIVE-only player-journey validations the stubbed gates can't see — ADR 0008/0012 two-window mirror + mid-gen join (`_verify_two_window_live.py`, live today), 0080 active-overseer (#766, `_verify_overseer_active_live.py`), and #829 turn-coalescing (auto-arms when its driver lands). Scheduled, **non-blocking** (`continue-on-error` per leg), key-gated on the repo secret **`OPENROUTER_API_KEY`** (a clean skip on a fork / missing secret).
...
`golden_path_glm-4.7.jsonl` is committed (regenerated #1251), so the `golden-path` PR job replays it
key-free — **advisory / non-gating** (excluded from `ci-gate.needs`, owner ruling 2026-07-12; runs + reports but does not block merge) (`frontend/INTEGRATION.md` §golden-path).
