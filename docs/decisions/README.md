...
| 0010 | [Token economy as architecture — metered LLM boundary, reasoning budget, cache-friendly prompts, non-degrading context tiering](./0010-token-economy-architecture.md) | Proposed (2026-06-21); investigation-driven (the cost trace: usage half-captured, reasoning never sent, caching left to luck, context capped-then-summarized). FE/adapter-only — the engine/closed set is untouched (ADR 0005); bounded by **non-degradation (#4)** as the binding constraint. Spec = feature 0069, built A→D. Mechanism to be built BDD/TDD-first |
