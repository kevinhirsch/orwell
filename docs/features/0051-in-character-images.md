...
> queue item). Mixed tier: prompt/descriptor plumbing is engine-side (TS gates), the
> generation pipeline and inline rendering are front-end (pytest + browser smoke). The gate
> split is decided at build time; nothing lands in `cucumber.cjs` until the engine half is
> built to green.
...
("the player is showing something to whoever is present"); 0051 is the output direction. The
obvious first deliverable is a **generated cast portrait set at season start** over the
Vault-free portrait descriptors that 0004/0020 already ship.
...
1. **Which moments may generate** — player-requested ("draw me the memory wall") vs.
   producer-beat (the engine offers an image at a key beat: move-in portraits, an eviction
   "camera still"). Both exist; neither is unbounded.
...
3. **Seed/style consistency per season** — the house must look like itself: a per-season
   ...
   houseguest renders recognizably across the season and across restarts.
4. **Cost/latency gating** — generation is the most expensive lever in the game; budgets
   ...
   an over-budget request is declined **in fiction** ("production is hoarding the cameras
   tonight").
5. **Graceful absence** — when no image-capable model is configured, the game plays
   identically: in-fiction refusal ("the feeds can't render that"), no error surface, no
   broken affordance. Mirror of E94(c).

## 3. The rules that bind (non-negotiable)
...
- **Knowledge scope:** an image may depict only what the **player knows** — witnessed events,
  ...
  witnessed is refused in fiction (suspicion is not footage). No pathway, no picture.
...
**In:** the per-season style anchor (seeded, persisted); the Vault-free image-prompt builder
over visible state + public appearance facets; the two generation triggers (player-requested,
producer-beat) with budget gating; the season-start cast portrait set (first deliverable,
feeding D9/E64's portrait surfaces); inline chat rendering; in-fiction decline/absence paths;
recording of image beats.
...
- **Engine/TS:** prompt-builder structural isolation (dependency-cruiser scope extension);
  ...
  budget constants honored; refusal on unwitnessed-scene requests; image beats recorded.
- **FE/pytest + smoke:** inline rendering; the no-image-model fallback (in-fiction copy, no
  error); the portrait set requested once per season start; no new chrome surface.

## 6. Definition of Done

All .feature scenarios green under their decided gates; the Vault-free prompt path proven
structurally (not by prompt wording); portraits stable within a season and varied across
seeds; play unaffected when generation is unavailable; the README index row flipped to Done.
