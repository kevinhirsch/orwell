# spec-only: this .feature is a design contract, NOT wired into the BDD gate (absent from cucumber.cjs paths; no step definitions). Its behavior is verified when implemented — the feature is unbuilt (Gate: — in the per-feature status index, docs/features/README.md); scenarios are drafted failing-first. (TEST-3, #628)
