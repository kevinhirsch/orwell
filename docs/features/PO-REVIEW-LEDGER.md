...
| 0031 | Game orchestrator & integrity watcher | ✅ Built | ✅ Approved (+ clarity note) | 2026-07-06 | The runtime backbone: **one `advance()` spine** = a committed turn (snapshot → run off-screen life + progression + consequence fold → verify → keep-or-revert). **Fail-closed integrity checkpoint** enforces the 4 mandates AT RUNTIME (non-degradation, Vault Wall on player+admin, eligibility legality, cross-user isolation) — a leaky/degrading advance is **rolled back, not saved**, prior save intact, fault logged (proven with a real leaking apply). Deterministic (same seed+ticks ⇒ byte-identical); logic-free supervisor; God-Mode health is Vault-free metadata only (player has no access). **PO Q&A:** clarified (1) the wall-clock watcher is **default-off** — house lives on the player's play-clock, one off-screen tick per turn; and (2) advance ≠ ceremony beat — beats are sparse plot milestones (~1/day), most turns are lingering that advance **in-game time-of-day (0066)** + hidden life without moving the beat. Added a **§8 clarity note** (default-off watcher, advance-vs-time two-track model, `circuitOpen` health field) so the spec matches the shipped ruling |
