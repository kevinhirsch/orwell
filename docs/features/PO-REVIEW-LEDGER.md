...
| 0006 | Outcomes by stats + temperature | ✅ Built | ✏️ Expand | 2026-06-28 | (3) **DONE+GREEN**: intent is now asked ONCE up front — removed the cosmetic per-round prompts (calibration-neutral, byte-identical outcomes; all BDD green). Also decided: keep the single-roll model (Path A) — Path B rebuild declined. STILL QUEUED: (1) tune favorite-win band DOWN toward ~60–70% and (2) NPCs carry a per-comp intent — both calibration-load-bearing, see build queue |
