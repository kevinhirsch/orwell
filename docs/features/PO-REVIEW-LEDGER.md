...
| 0025 | Reserve twists | ✅ Built | 🔧 Approved + Expanded (rebuilt) | 2026-07-06 | PO found the original shipped only 1 of 3 pooled twists (double eviction), pre-scheduled at setup. **Redesigned + rebuilt to spec:** all three twists (double eviction, secret power, battle-back) are live in a **standing reactive pool** — each fires when the live house EARNS its trigger, on **per-season dynamic thresholds** (no cross-playthrough predictability), with a **hold-back** roll (some seasons quiet), **all three may fire in one season** (each ≤ once, cooldown-spaced). **Player plays their own** (secret-power decision; battle-back competes). Secret power = one-time safety off the block (still one eviction); battle-back = a juror re-enters yet the jury still closes at 9. Sealed from player+admin, format-preserving, deterministic, durable. Gated behind `ORWELL_REACTIVE_TWISTS` (off ⇒ byte-identical legacy path; calibration baseline untouched). Gates: `reserveTwistsReactive.test.ts`, `reactiveTwistsLive.test.ts`, reactive `.feature` (17 scenarios) |
