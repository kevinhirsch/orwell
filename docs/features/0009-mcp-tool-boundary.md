...
> **MVP seam** between the TypeScript engine and the odysseus front-end/agent (`frontend/`).
...
This is the **integration linchpin** for the MVP. odysseus's **agent** (Python MCP client,
...
```
 odysseus (Python: UI + LLM + agent)  ──MCP──►  bbai engine (TS: MCP server)
...
(`recordInteraction`). **Feeds** the odysseus agent integration (`frontend/INTEGRATION.md`), the
