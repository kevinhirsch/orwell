...
| 0068 | [Admin "Public deployment / Connect to the internet" (UI-driven exposure)](./0068-admin-public-deployment.md) | FE + scripts *(planned)* | 📝 Spec only (ADR 0007; drives 0067) · an admin Settings→System panel + a token-paste **Connect** wizard using a Cloudflare **remotely-managed** tunnel, applied through the existing privileged **ops** mechanism (flag→root oneshot→script: `.env` upsert + cloudflared install/run + FE restart + token shred) with live tunnel/URL status. Engine **unchanged**; Cloudflare-account steps stay dashboard-side |
