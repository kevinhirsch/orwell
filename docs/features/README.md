...
| 0058 | [Deep character profiles (born deep, persist, play out)](./0058-deep-character-profiles.md) | **Behavioral fidelity / Vault Wall** | **Phase 1 (engine foundation) — BDD-gated.** Born-deep §3 profile split across the wall: PUBLIC `biography` (multi-sentence) + structured `physicalCharacteristics` facet (byte-stable on the Character, on the view/`npcVoice`/roster); HIDDEN 2–3 secrets + true goals + weakness + Day-1 perception sealed in the Vault & soul-indexed (full-fidelity recall, L27b), NEVER on any player/admin surface. Day-1 read seeds the NPC→player edge; story threads (v0 §15 shape) seeded+sealed+persisted, with the activation+fold hook reusing the 0023 consequence loop. `recordCastProfile` write-back seam typed + stubbed. `deepProfile.ts` · `0058-deep-character-profiles.feature` · `deepProfile.test.ts` / `.property.test.ts`. **Deferred Phase 2:** LLM write-back wiring, portrait consumption (L29), premiere voicing (L31), thread trigger/resolution scheduler ([0060](./0060-story-thread-scheduler.md) specs the scheduler) |
| 0061 | [Player self-eviction (voluntary walk-out / quit)](./0061-player-self-eviction.md) | Gameplay / player agency | 📝 **SPEC ONLY — not built.** The genuine, RECORDED, confirmed voluntary quit on top of **0046** (reuses its terminal/jury machinery), the **0034** seam (a new explicitly-confirmed, player-level/OOC `self-evict` decision — never parsed from an in-character line), the **0023** consequence fold, **0047**'s player-authored goodbye, and **0048**'s pre-jury recap. Closes the **L39(a)** gap: the GM's interim refusal becomes correct only for *unconfirmed/ambiguous* exits; 0061 is the sanctioned *confirmed* path through the one exit door — no second eviction/restart hinge, Vault-free throughout. Open owner calls: jury eligibility on a quit (recommend forfeit/exit), the parting-message offer, when-a-quit-can-fire timing, NPC self-eviction (deferred). `0061-player-self-eviction.feature` (skeleton; not yet in `cucumber.cjs`) |
| 0060 | [Story-thread trigger/resolution scheduler (0058 Phase 2)](./0060-story-thread-scheduler.md) | **Behavioral fidelity / Vault Wall** | 📝 **SPEC ONLY — not built** (owner-approved, restrained posture; build deferred until current lanes + any R3/SQLite per-tick-budget work land). The *when-to-fire* orchestrator that makes 0058's seeded threads PLAY OUT: a small seeded scheduler rides the EXISTING off-screen tick and walks each thread `dormant → active` (reuse 0023 `activateThread`) → `surfaced` (reuse 0038 gossip + 0002 pathway, anchored belief only) → `resolved`, or → `expired` (recorded, never deleted). Adds a structured `triggerCondition` enum (alongside the prose trigger), a bounded `threadConstants.ts`, and a **hard season surfacing cap ≈3–4** (the L40 restraint lesson; NPC↔NPC surfacing far more common than to the player; belief-level/atmospheric this phase). Authors nothing — purely orchestration. `0060-story-thread-scheduler.feature` (skeleton; not yet in `cucumber.cjs`) |
