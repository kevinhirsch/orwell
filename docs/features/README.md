...
| 0081 | [Narration-faithfulness gate (the overseer's 2nd role)](./0081-narration-faithfulness-gate.md) | spec-only | 📝 Spec · the **faithfulness role 0079 deferred** — checks the narration is faithful to engine truth (board / persona / leak / omission). **Hybrid** detection: the deterministic 0065 guard flags **claim-bearing turns**; an LLM judge runs only on those, post-stream. Folds into the overseer's 3-state mode + OVERSEER log. Correction is **clever & diegetic, split on ADR 0005** — an **open-set** slip is *adopted* canonical (the claim becomes true), a **closed-set** slip is *reframed* in-fiction (the outcome is **never** bent; the anti-sycophancy guard is `adopt ⇒ open-set`); un-reframable ⇒ backend re-ground + log, no visible retraction. Full faithfulness everywhere — in-game loop **and** the casting/premiere/preview junctions (subsumes a separate 0082). Opt-in (its **own** off/shadow/active dial), live-only, fail-soft. Owner-ruled 2026-06-24 (boundary held — `adopt` open / `reframe` closed, never bend a consequential outcome; own dial; configurable un-reframable fallback; O3 the lone impl detail). Design note + provisional `.feature`; not in `cucumber.cjs`. |
