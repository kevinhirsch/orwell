...
| 0075 | [Trust-gated confidences](./0075-trust-gated-confidences.md) | built (engine) | 🟢 Engine built (2026-06-24) · the player-driven `confide` lever ships end-to-end — a houseguest confides a real secret only when they have a *reason* (bond/favor/debt), with a graduated disclosure ladder, and a manipulative houseguest may plant a false confidence (capped). Vault Wall held structurally (the engine decides + records every disclosure; the model only voices). Gate: `confidence.test.ts` + `confideBoundary.test.ts` (MCP-boundary + Vault-wall). Opt-in by construction ⇒ calibration byte-identical. Fast-follows: wire the full `.feature` into `cucumber.cjs`, the passive lie-catch (spec open-Q #4), the FE confide-under-call guardrail. |
...
| 0077 | [House map, privacy & eyeshot](./0077-house-map-privacy-and-eyeshot.md) | building (phased) | 🟢 Phase 1 (the recent-BB 13-room floor plan + movement adjacency) **merged** (PR #772). Phase 2a (the **SIGHTLINE graph** — eyeshot < adjacency: `whereabouts` is sightline-scoped, so a closed door one room over no longer leaks its occupants; `HOUSE_SIGHTLINE`/`areVisible`/`isPrivateRoom`) **built 2026-06-24** (`sightline0077.test.ts` + `whereaboutsSightline.test.ts`; read-side only ⇒ calibration byte-identical). Phase 2b next: tracked-occupancy + conspicuousness + sub-zone earshot, then wire the `.feature` into `cucumber.cjs`. Sibling of 0076. |
| 0078 | [Motivated off-screen society & intentional movement](./0078-motivated-society-and-intentional-movement.md) | spec-only | 📝 Spec · co-presence becomes **goal-driven** (relationship / deal / bloc / agenda-motivated movement) instead of arbitrary, so "who's in a room together" is meaningful; the society stays hidden — only an overheard fragment or a witnessed meeting reaches the player, through a pathway. Sibling of 0077; depends on 0049/0017/0026/0040/0044/0041/0038/0066; planned gate re-verifies the `juryReach` + gradient calibration on the new movement model. Design note + `.feature`; not yet wired into `cucumber.cjs`. |
