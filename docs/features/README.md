...
| 0079 | [Runtime loop overseer & diagnostic log](./0079-runtime-overseer-and-diagnostic-log.md) | spec-only | 📝 Spec · a reasoning overseer for the engine↔LLM loop, summoned **only** by a sparse symptom-gate (never every turn); on wake it diagnoses root cause from **Vault-free** telemetry and unsticks with a small, fixed, mandate-safe toolbox (nudge→advance · constrained record · re-inject `stateDelta` · hold · escalate) — **wide eyes, small hands**. Every wake logs to a Vault-free "Overseer (live)" admin ring (a 4th G1b log source). Behind `OverseerPort` + a deterministic stub (fail-soft; seeded lanes never call a model); the 0031 checkpoint stays the floor. Governed by ADR 0003/0005. Design note + `.feature`; not yet wired into `cucumber.cjs`. |
