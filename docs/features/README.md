...
| 0058 | [Deep character profiles (born deep, persist, play out)](./0058-deep-character-profiles.md) | Behavioral fidelity / characters — **the true-sim spine** | **📝 SPEC (design)** — umbrella for L28b/L27b/L29/L31 (+seeds L35). LLM-authored §3-depth profiles (biography · voice register · named weakness · true goals · 2–3 secrets · Day-1 player read · own physical-characteristics facet), split across the Vault Wall, persisted **airtight as a fixed baseline** that **only grows** (soul evolves from it, 0041); secrets become **live story threads** (trigger → organic surfacing → hidden-weight fold → resolution) reusing 0023/0038/0002/0025; emergent + divergent per seed. Seeded floor (diversity caps + non-mirroring + demeanor) already ships; the authoring/threads/recall layer is the build. BDD-first `.feature` owed |
