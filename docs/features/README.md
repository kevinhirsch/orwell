...
| 0069 | [Token economy: metered LLM boundary, reasoning budget & non-degrading context tiering](./0069-token-economy-and-context-budget.md) | FE (planned) | 📝 Spec only (ADR 0010) · meter the one LLM boundary (full usage envelope: input/cached/reasoning/output/cost + context-%, Vault-free, admin-only) → token **policy per call class** (reasoning effort + output cap + caching + budget; finally *send* `reasoning`) → cache-friendly stable-prefix-first assembly + per-session stickiness → **non-degrading context tiering** (escalate toward the window before lossy compaction). FE/adapter-only; engine/closed set untouched; player never sees a number. Built A→D. Gate (planned): `frontend/tests/test_adr0010_*.py` (recorded deviation, like 0066) |
