...
| 0056 | [Season-to-season character continuity (soft re-run of casting)](./0056-season-character-continuity.md) | Player UX / lifecycle | **Engine + restart-door relay built** — `keepCharacter` carries the prior houseguest into a new cast through the one D1/R1 hinge (seed-independent CHARACTER ⇒ byte-faithful); `/new-game` + relay thread it. Unit-gated (`seasonContinuity.test.ts`) + FE pytest (`test_0056_keep_character.py`). In-chat keep/recreate UX + per-season portrait toggle to follow |
| 0057 | [Seasons as levels (finale lobby, next-season invite, progress)](./0057-seasons-as-levels.md) | Player UX / lifecycle / progression | **SPEC + chunk 1 (season-number foundation) built** — per-user FE season store + `GET /season`, player-reachable `POST /next-season` (gated on a finished season; increments) & `POST /reset-progress` (no increment), both through the one sanctioned reset. FE pytest (`test_0057_seasons.py`). Chunks 2–4 (the ≤5px season progress bar + "Season N" chip, the persistent finale "New season" surface, the producers' re-approach nudge) to follow |
