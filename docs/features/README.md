...
| 0070 | [Off-screen society texture enrichment](./0070-offscreen-texture-enrichment.md) | Engine + FE | 📝 Spec (Hermes integration Wave 1; `docs/HERMES-INTEGRATION-PLAN.md` A1) · enrich the **prose** of already-recorded **hidden** off-screen scenes with model-voiced texture via a new FE-driven write-back `recordOffscreenSceneTexture`, fanned out in parallel (the hermes `delegate_tool.py` subagent **pattern**, MIT © Nous Research — pattern only). Enrichment lands *downstream* of the Vault boundary (event already hidden); reaches the player only via existing overhear/gossip pathways. Engine keeps the entire closed set (ADR 0005); no texture ⇒ byte-identical fold (`expressiveNonCollapse`). Mandate #1 fidelity win |
| 0071 | [Defensive hardening: log redaction & URL/path guards](./0071-defensive-hardening-redaction-url-guards.md) | FE | 📝 Spec (Hermes integration Wave 2; B1) · adapt `agent/redact.py` (broader secret masking on the FE log path) + the gateway SSRF / path-traversal URL guards (fail-closed). Mandate-neutral defense-in-depth; no Vault surface. Attribution: Nous Research MIT |
| 0072 | [Multi-platform gateway: play your game from messaging platforms](./0072-multi-platform-gateway.md) | FE | 📝 Spec (Hermes integration Wave 3; A2/A3 — **in scope** per owner ruling 2026-06-22) · a new non-Vault player adapter (`frontend/gateway/`) over the same Vault-free MCP player channel: per-platform adapter+registry + `pairing.py` identity binding (one human ⇒ one game across platforms, 0021) + a **server-side reasoning/operator-aside scrub chokepoint** (the accordion-less-transport safety) + decision-card text degradation + `ORWELL_GAME_BUILD` tool-gating. Adapts hermes `gateway/` (MIT © Nous Research). Coordinates with ADR 0007 / 0064 |
| 0073 | [Structural anti-sycophancy wall: game-build boundary is a CI gate](./0073-structural-anti-sycophancy-wall.md) | FE tests + CI | 📝 Spec (Hermes integration audit, 2026-06-22) · makes the `ORWELL_GAME_BUILD` game-build wall **structurally proven in CI**, closing the asymmetry with the Vault Wall (compile-time via dependency-cruiser). A new `frontend/tests/test_game_build_wall.py` gate — env-pinned (forces game build on internally), covering set purity, context-injection zeroing, JS-strip completeness, and HTTP-level 404 for every drop-set route. Wired into the CI frontend job as a required step |
