---
name: orwell
description: Use when the user asks Claude Code to read or write Orwell data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Claude Agent API. Requires ORWELL_URL and ORWELL_API_TOKEN.
---

# Orwell

Use this skill when a user asks to interact with Orwell from Claude Code.

## Configuration

Expect these environment variables:

- `ORWELL_URL`: Base URL for the user's Orwell instance, for example `http://127.0.0.1:7000`.
- `ORWELL_API_TOKEN`: Scoped API token created in Orwell Settings > Integrations > Add Integration > Claude Agent.

If either value is missing, do not guess credentials. Tell the user to create a Claude Agent token in Orwell Settings and expose both values to the terminal session.
...
- **Calendar event ("meeting at 3pm", "dentist Tuesday 10am")** → calendar event. Use for scheduled time blocks, meetings, appointments, recurring schedules. These show up on the calendar grid; reminders for them are configured separately in Orwell settings.
...
- All Orwell data access MUST go through the scoped HTTP API under `/api/codex/*` (the canonical scope-gated agent API, shared by all agent integrations).
...
- Do not use SSH, Docker, direct Python imports, SQLite queries, MCP internals, browser cookies, or local files to read/write Orwell user data.
...
python3 ~/.claude/skills/orwell/scripts/orwell_api.py todos list
python3 ~/.claude/skills/orwell/scripts/orwell_api.py todos add "Follow up"
```
...
```bash
python3 ~/.claude/skills/orwell/scripts/orwell_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.claude/skills/orwell/scripts/orwell_api.py emails list 5
python3 ~/.claude/skills/orwell/scripts/orwell_api.py emails read UID
```
...
```bash
python3 ~/.claude/skills/orwell/scripts/orwell_api.py GET /api/codex/memory
python3 ~/.claude/skills/orwell/scripts/orwell_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
python3 ~/.claude/skills/orwell/scripts/orwell_api.py cookbook tasks

# Tail the failing one (sessionId from `cookbook tasks`)
python3 ~/.claude/skills/orwell/scripts/orwell_api.py cookbook output serve-abc12345 400

# Stop the previous attempt before you try a new flag set
python3 ~/.claude/skills/orwell/scripts/orwell_api.py cookbook stop serve-abc12345

# Relaunch with new flags. cmd MUST begin with one of the allowlisted binaries.
python3 ~/.claude/skills/orwell/scripts/orwell_api.py cookbook serve \
...
If you are about to reach the Orwell host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Orwell Settings and token scopes. Ask the user to enable the relevant Claude Agent tool toggle instead.
