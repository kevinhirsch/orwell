...
MCP_DISABLED = os.environ.get("ORWELL_DISABLE_MCP", "").lower() in ("1", "true", "yes")


async def register_builtin_servers(mcp_manager):
    """Connect all built-in MCP servers to the manager."""
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via ORWELL_DISABLE_MCP")
        return
        ...
        for server_id, cfg in _BUILTIN_NPX_SERVERS.items():
            ...
            if pkg_spec and not await _is_npx_package_cached(npx_path, pkg_spec):
                logger.warning(
                    ...
                    f"          (run once, then restart Orwell)\n"
