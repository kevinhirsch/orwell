#!/usr/bin/env python3
"""Orwell — first-time setup script.
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("ORWELL_ADMIN_USER", "").strip().lower()
        password = os.getenv("ORWELL_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — use them directly
            pass
        elif sys.stdin.isatty() and not os.getenv("ORWELL_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("ORWELL_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("ORWELL_ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set ORWELL_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def main():
    ...
    if not os.getenv("ORWELL_SKIP_RUN_HINT"):
