...
# a switchable 'Demo' account in Orwell + fake seed mail.
#
...
DOCKER_DIR="${ORWELL_DEMO_MAIL_DIR:-$HOME/docker/snappymail}"
USERS_FILE="$DOCKER_DIR/dovecot/conf/users"
DEMO_USER="demo@orwell.local"
DEMO_PASS="demodemo"
...
case "${1:-}" in
  setup)
    ...
    echo
    echo "Done. In Orwell, switch to the 'Demo' account to show off the inbox."
    ;;
  reseed)
    "$PY" "$HERE/seed_demo_emails.py" --reset
    ;;
