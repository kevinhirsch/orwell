  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && !hasCustomUpdate) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Orwell.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('Orwell app', 'Run inside the Orwell app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="margin-top:8px;padding-top:6px;border-top:1px solid color-mix(in srgb, var(--border) 50%, transparent);font-size:9.5px;opacity:0.65;text-align:right;">'
    + 'Don\'t see a model? '
    + '<a href="https://github.com/kevinhirsch/orwell/discussions/1962" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;">'
    + 'Request it →'
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Orwell capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Orwell keys, choose model directories, and set the default server. Local is this machine.</p>';
