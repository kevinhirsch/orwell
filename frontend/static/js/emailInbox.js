...
const _acct = () => window.__orwellActiveEmailAccount
  ? `&account_id=${encodeURIComponent(window.__orwellActiveEmailAccount)}`
  : '';
...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const lastSeen = parseInt(localStorage.getItem('orwell-email-last-seen-uid') || '0', 10);
  ...
  try {
    ...
    fetch(`${API_BASE}/api/email/list?folder=INBOX&limit=1${_acct()}`)
      ...
      .then(data => {
        ...
        if (emails.length > 0) {
          const maxUid = Math.max(...emails.map(e => parseInt(e.uid, 10) || 0));
          localStorage.setItem('orwell-email-last-seen-uid', String(maxUid));
        }
        const dot = document.getElementById('email-unread-dot');
        if (dot) dot.style.display = 'none';
      })
