...
function _publishActiveAccount() {
  try { window.__orwellActiveEmailAccount = state._libAccountId || null; } catch (_) {}
...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete Orwell reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show Orwell reminder emails">
  ...
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all Orwell reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/orwell/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
...
const _BUBBLES_DISABLED_KEY = 'orwell.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'orwell.email.summaryCollapsed';
