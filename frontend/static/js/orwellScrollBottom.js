...
// reuses the same threshold). Clicking it re-enables auto-scroll and jumps to the newest message;
// it hides itself the moment the reader is back at the bottom. An optional unread badge counts how
// many messages landed while the reader was scrolled away, clearing on click / on reaching bottom.
//
...
(function () {
  ...
  function ensureCss() {
    ...
    st.textContent =
      "#" + BTN_ID + " {" +
      "  position: absolute; right: 18px; bottom: 96px; z-index: 40;" +
      "  width: 38px; height: 38px; border-radius: 50%;" +
      "  display: flex; align-items: center; justify-content: center;" +
      "  background: color-mix(in srgb, var(--panel, #1b1b1b) 88%, transparent);" +
      "  color: var(--fg, #cfd8e3);" +
      "  border: 1px solid color-mix(in srgb, var(--accent, var(--border, #355a66)) 45%, transparent);" +
  ...
  function ensureBtn() {
    ...
    // The container is position:relative-capable chrome; appending keeps the button anchored to
    // the chat area (above the input bar) rather than the whole viewport.
    container.appendChild(_btn);
    _btn.addEventListener("click", function () {
      try {
        if (window.uiModule && window.uiModule.setAutoScroll) window.uiModule.setAutoScroll(true);
        if (window.uiModule && window.uiModule.scrollHistoryInstant) window.uiModule.scrollHistoryInstant();
        else if (window.uiModule && window.uiModule.scrollHistory) window.uiModule.scrollHistory();
        else { var b = chatBox(); if (b) b.scrollTop = b.scrollHeight; }
      } catch (_) {}
      clearUnread();
      update();
    });
