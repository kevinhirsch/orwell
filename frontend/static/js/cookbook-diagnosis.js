  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume Orwell owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## Orwell Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
