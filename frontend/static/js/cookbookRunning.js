...
function _winSessionCmd(task, tmuxArgs) {
  ...
  const sd = host ? '$env:TEMP\\orwell-sessions' : '$env:TEMP\\orwell-tmux';
  ...
  if (tmuxArgs.includes('capture-pane')) {
    ...
    const ps = host
      ...
      : `Get-Content (Join-Path $env:TEMP 'orwell-tmux\\${sid}.log') -Tail ${lines} -ErrorAction SilentlyContinue`;
  ...
  if (tmuxArgs.includes('has-session')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'orwell-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Get-Process -Id $p -ErrorAction SilentlyContinue | Out-Null; if ($?) { exit 0 } else { exit 1 } } else { exit 1 }`;
    return host ? `ssh ${pf}${host} "powershell -Command \\"${ps}\\""` : `powershell -Command "${ps}"`;
  }
  if (tmuxArgs.includes('kill-session')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'orwell-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Stop-Process -Id $p -Force -ErrorAction SilentlyContinue }; Remove-Item (Join-Path $env:TEMP 'orwell-tmux\\${sid}.*') -Force -ErrorAction SilentlyContinue`;
    return host ? `ssh ${pf}${host} "powershell -Command \\"${ps}\\""` : `powershell -Command "${ps}"`;
  }
  if (tmuxArgs.includes('send-keys') && tmuxArgs.includes('C-c')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'orwell-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Stop-Process -Id $p -ErrorAction SilentlyContinue }`;
    return host ? `ssh ${pf}${host} "powershell -Command \\"${ps}\\""` : `powershell -Command "${ps}"`;
  }
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      _updateTask(task.sessionId, {
        status: 'running',
        output: `${task.output || ''}\n\n[orwell] Retrying download. Progress may briefly look like a fresh download while HuggingFace checks cached/incomplete files; cached partial files will be reused when available.`.trim(),
        _retrying: true,
      });
      ...
      menuBtn.addEventListener('click', (e) => {
        ...
        if (_isWindows(task)) {
          ...
          const logCmd = host
            ...
            : `powershell -Command "Get-Content (Join-Path $env:TEMP 'orwell-tmux\\${task.sessionId}.log') -Wait"`;
