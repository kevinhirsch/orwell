  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('orwell-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('orwell-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('orwell-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('orwell-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('orwell-char-sessions', JSON.stringify(charSessions));
