...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the Orwell container by design. The image ships no "
...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside Orwell."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside Orwell.",
        )
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "orwell-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f'ORWELL_USER_SHELL="${{SHELL:-}}"\n'
        f'if [ -n "$ORWELL_USER_SHELL" ] && [ -x "$ORWELL_USER_SHELL" ]; then\n'
        f'  ORWELL_USER_PATH="$("$ORWELL_USER_SHELL" -ic \'printf "__ORWELL_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__ORWELL_PATH__//p\' | tail -n 1 || true)"\n'
        f'  if [ -n "$ORWELL_USER_PATH" ]; then export PATH="$ORWELL_USER_PATH:$PATH"; fi\n'
        f"fi\n"
