...
def _apply_orwell_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Orwell-Origin"] = ORWELL_MAIL_ORIGIN
    if kind:
        msg["X-Orwell-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Orwell-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Orwell marker header, but include the subject
                # fallback too. The fallback uses a distinct Orwell prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Orwell-Kind "reminder" SUBJECT "Reminder (Orwell):"{from_clause})',
                )
    ...
    @router.delete("/orwell/reminders")
    async def delete_orwell_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Orwell reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Orwell-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Orwell):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Orwell):")})'))
                            # Legacy reminders created before the Orwell
            ...
            logger.error(f"delete_orwell_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", orwell_kind=None, orwell_ref=None,
    ):
        ...
        _apply_orwell_headers(outer, orwell_kind or "scheduled", orwell_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, orwell_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("orwell_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="orwell.local")
        ...
        if req.orwell_kind:
            _apply_orwell_headers(outer, req.orwell_kind)
