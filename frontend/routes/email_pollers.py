                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Orwell (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Orwell" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "orwell_kind" if "orwell_kind" in cols else "'scheduled' AS orwell_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                orwell_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Orwell-Origin"] = "orwell-ui"
                outer["X-Orwell-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", orwell_kind or "scheduled")[:64]
                outer["X-Orwell-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `orwell-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `ORWELL_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `orwell-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("ORWELL_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `ORWELL_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (ORWELL_INPROCESS_POLLERS=0); "
            "drive `orwell-mail poll-scheduled` externally."
        )
        return
    import asyncio
