...
def _container_loopback_reachable(base_url: str, timeout: float = 0.2) -> bool:
    ...
    This distinguishes "a model server running alongside Orwell in the same
...
def _rewrite_loopback_for_docker(base_url: str, *, container_local: bool = False) -> str:
    ...
    default) otherwise targets the Orwell container itself, so the probe gets
    a connection error and the endpoint is rejected with a misleading "No
    models found for that provider/key".

    Cookbook local serves are the opposite case: Orwell started the model
        ...
        if 300 <= r.status_code < 400:
            ...
            if loc.startswith("/login") or "/login" in loc:
                return {
                    ...
                    "error": "That is Orwell, not a model server. Use the Ollama URL, usually http://host.docker.internal:11434/v1 in Docker.",
                }
...
def setup_model_routes(model_discovery):
    ...
    def create_model_endpoint(
        ...
        # server. Cookbook local serves are launched inside Orwell itself, so
