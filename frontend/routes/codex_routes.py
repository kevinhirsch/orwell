...
reuse existing Orwell helpers and enforce API-token scopes before touching
user data.
"""
        ...
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for path in sorted(root.rglob("*")):
                if path.is_dir() or "__pycache__" in path.parts or path.suffix == ".pyc":
                    continue
                zf.write(path, Path("orwell") / path.relative_to(root))
        buf.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="orwell-codex-plugin.zip"'}
...
def setup_claude_routes() -> APIRouter:
    ...
    def plugin_zip(request: Request):
        ...
        buf.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="orwell-claude-skill.zip"'}
        return StreamingResponse(buf, media_type="application/zip", headers=headers)
