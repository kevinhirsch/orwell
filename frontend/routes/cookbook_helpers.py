...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Orwell runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Orwell' own interpreter directory to PATH. If Orwell itself is
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'ORWELL_USER_SHELL="${SHELL:-}"',
        'if [ -n "$ORWELL_USER_SHELL" ] && [ -x "$ORWELL_USER_SHELL" ]; then',
        '  ORWELL_USER_PATH="$("$ORWELL_USER_SHELL" -ic \'printf "__ORWELL_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__ORWELL_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$ORWELL_USER_PATH" ]; then export PATH="$ORWELL_USER_PATH:$PATH"; fi',
        'fi',
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Orwell.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$ORWELL_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $ORWELL_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$ORWELL_PREFLIGHT_EXIT"')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Orwell-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('ORWELL_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$ORWELL_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  ORWELL_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[orwell] vLLM CLI: $ORWELL_VLLM_BIN"')
    runner_lines.append('  ORWELL_VLLM_VERSION="$("$ORWELL_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$ORWELL_VLLM_VERSION" ]; then echo "[orwell] vLLM version: $ORWELL_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('ORWELL_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $ORWELL_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $ORWELL_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $ORWELL_CMD_EXIT ==="')
        runner_lines.append('exit "$ORWELL_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[orwell] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('      _orwell_has_cudart() {')
    ...
    runner_lines.append('      }')
    runner_lines.append('      if _orwell_has_cudart; then')
    runner_lines.append('        echo "[orwell] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[orwell] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[orwell]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[orwell]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    else')
    runner_lines.append('      echo "[orwell] WARNING: no HIP/CUDA toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[orwell]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[orwell]   Install ROCm for AMD GPUs or vLLM/CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
...
def _llama_cpp_rebuild_cmd() -> str:
    ...
    return (
        ...
        'echo "[orwell] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\orwell-sessions"
