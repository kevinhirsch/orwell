...
export const ADMIN_TOOLS: readonly ToolDescriptor[] = [
  ...
  { name: "setBehavioralFlags", channel: "admin/God Mode", readsVault: false, description: "B2: turn one or more 'living house' behavioral-fidelity layers ON or OFF at runtime — no engine restart. { campaigns?, trajectories?, triggers?, secretPacing?, juryHouse?, seededTieSurfacing?: boolean } — each field optional; absent ⇒ that layer's current setting is untouched. Overrides the ORWELL_CAMPAIGNS/ORWELL_TRAJECTORIES/ORWELL_TRIGGERS/ORWELL_SECRET_PACING/ORWELL_JURY_HOUSE/ORWELL_SEEDED_TIE_SURFACING env defaults for this sandbox; resets on restart (the FE re-applies the persisted setting on boot). Every layer is calibration-proven-neutral-when-off. Vault-free." },
