 ...
 * (unlike a belief's `subject`, which the diffusion does not propagate), so ANY holder's belief resolves
 * back to WHICH houseguest is credited as reliable — one home for the format the seed side (the reputation
 * tick) and the read side (the NPC deal-willingness nudge / the registry reader) both depend on. Pure data.
 */
