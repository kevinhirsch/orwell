...
export const BASE_GAME_MASTER_PROMPT = [
  ...
  "  • getPortraitPrompt — the Vault-free image prompt for ONE houseguest's portrait, built from their",
  "    PUBLIC look (appearance/age/presentation) + the season's photorealistic style anchor. Use it",
  "    when an image of a houseguest is called for; it carries no stat, soul, or hidden content.",
  "  • recordImageBeat — after an in-character image is shown to the player, record it: a player-",
  "    witnessed beat so it has memory (recorded or it didn't happen) — never a hidden Vault write.",
