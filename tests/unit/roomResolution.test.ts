...
describe("resolveRoom — the natural-name → room-id contract (the bug fix)", () => {
  ...
  it("resolves the common natural names the narrator says (the real-log cases)", () => {
    const cases: Array<[string, Room]> = [
      ...
      ["kitchen", "kitchen"],
      ["KITCHEN", "kitchen"],
      ["dining room", "dining-room"],   // 0077: the dining area is its own room
      ["dining", "dining-room"],
      ["hallway", "hallway"],           // 0077: the private-wing corridor
      ["hall", "hallway"],
      ["lounge", "lounge"],             // 0077: the lounge is its own room now (was → living room)
      ["games room", "lounge"],
      ["backyard", "backyard"],
      ["back yard", "backyard"],
      ["yard", "backyard"],
      ...
      ["bedroom a", "bedroom-a"],
      ["Bedroom B", "bedroom-b"],
      ["bedroom c", "bedroom-c"],       // 0077: the third bedroom
    ];
  ...
  it("never silently fails the way the narrator's guessed 'bedroom' did — a bare 'bedroom' always resolves to a real bedroom", () => {
    ...
    if (noCtx.kind === "ambiguous") {
      expect([...noCtx.candidates].sort()).toEqual(["bedroom-a", "bedroom-b", "bedroom-c"]);
    }
