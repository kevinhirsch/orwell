...
describe("0038 — live off-screen society", () => {
  ...
  it("the player is shown no hidden scene content", () => {
    ...
    for (const c of sb.engine.events.query().filter((e) => e.hidden).map((e) => e.content)) {
      expect(visible.includes(c)).toBe(false);
    }
  });

  it("is seed-deterministic for the same user", () => {
    const a = startedSociety("u", 4, 5).sb.engine.events.query().map((e) => e.content).join("|");
    const b = startedSociety("u", 4, 5).sb.engine.events.query().map((e) => e.content).join("|");
    expect(a).toBe(b);
  });
});
