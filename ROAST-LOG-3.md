...
- **Evidence:** at S2 casting the engine reported `casting.ready=True` with the full profile captured (`playerName`, backstory, archetype, strategy, 8 interview notes; `next: optional cast photo`), yet the model kept interviewing for 6+ turns ("Three times. Same line… every wall has a door"), never calling `createCharacter`. It finalized only once the player sent an explicit *"lock it in, start the season, skip the photo"* readiness signal. (S1's *Marcus* finalized in ONE turn because his opening line was already assertive/complete.)
...
### CARRY-4 · `[NIT — housekeeping]` · An auditor (this session) used the banned legacy name "Marcus Webb" (and "Priya Anand") as live test personas in `ROAST-LOG-3.md` / `docs/audits/2026-06-19-live-debug-issues.md` — docs only, no product impact, but against the "roles not names" testing rule.
